...
!     Model parameters
      ...
      logical  :: use_chl,use_cyanos,couple_co2,not_0d
      contains
...
!BOC
   ...
   blight  = max( ((flachl/fla)*self%alfaPs*par) / sqrt((self%BioC(2)*sedy0)**2+(flachl/fla)**2*self%alfaPs**2*(par**2)) ,0.0_rk)
   Ps_prod = Ts * min(blight, up_n, up_pho)
   blight  = max( ((diachl/dia)*self%alfaPl*par) / sqrt((self%BioC(1)*sedy0)**2+(diachl/dia)**2*self%alfaPl**2*(par**2)) ,0.0_rk)
